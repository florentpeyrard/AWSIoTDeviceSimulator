...
//angular-toArrayFilter : to make Angular support objects as well as arrays
    angular.module('simulatorApp', ['angular-toArrayFilter'])
      .controller('SimulatorController', ['$scope',function($scope) {
        ...
        $scope.newThing = {"thingName":"","thingTypeName":""};
...
/*        $scope.updateModel = function(variable,value){
                        ...
                        for (i=0;i<data.things.length;i++){
                            ...
                            $scope.things[i]['desired']=[];
                                ...
                                thing.device
                                    ...
                                    thing.device.subscribe(topic,{qos:$scope.qos},function(err,granted){
                                                   ...
                                                   //$scope.log('subscribed topics : '+JSON.stringify(granted));
                                                    ...
                                                    thing.device.publish('$aws/things/'+thing.thingName+'/shadow/get',null,{qos:$scope.qos});         
        ...
        $scope.addProperty = function() {
            ...
            if (count>1) $scope.log('More than one thing selected. Please select one thing only.');
                ...
                $scope.things[selectedThing].state.push({propName : $scope.newPropertyName, propValue : ""});
                $scope.things[selectedThing].reported.push({propName : $scope.newPropertyName, propValue : ""});
                $scope.things[selectedThing].desired.push({propName : $scope.newPropertyName, propValue : ""});
                $scope.send();
            }
         ...
         $scope.send = function() { 
            for (var i=0;i<$scope.things.length;i++){
                ...
                $scope.things[i].device.publish('$aws/things/'+$scope.things[i].thingName+'/shadow/update',JSON.stringify(doc),{qos:$scope.qos});
            }                                               
         }
                       ...
                       //thing creation on AWS
                        ...
                        iot.createThing(params, function(err, data) {
                            if (err) $scope.log(err); 
                                ...
                                $scope.things.push({thingName:newThingName});
                                $scope.buildThingIndex();
                        ...
                        doc.state.reported[$scope.things[selectedThing].state[j].propName] = $scope.things[selectedThing].state[j].propValue;
                                                                ...
                                                                for(var prop in doc.state.reported){
                                                                    ...
                                                                    $scope.things[index].device.publish('$aws/things/'+$scope.things[index].thingName+'/shadow/update',JSON.stringify(doc),{qos:$scope.qos});  
                                    ...
                                    if (thing){
                                       ...
                                       if (topic.match(/get\/accepted/) != null){
                                           ...
                                           for (var prop in data.state.reported){
                                               ...
                                               if (!foundInState){
                                                   thing['state'].push({propName : prop, propValue : data.state.reported[prop]});
                                               }
                                               ...
                                               if (!foundInReported){
                                                   thing['reported'].push({propName : prop, propValue : data.state.reported[prop]});
                                               }
                                           ...
                                           for (var prop in data.state.desired){
                                               ...
                                               if (!foundInDesired){
                                                   thing['desired'].push({propName : prop, propValue : data.state.desired[prop]});
                                               }                                                         
                                                ...
                                                for (var i=0;i<thing.reported.length;i++){
                                                    if (thing.reported[i].propName == prop){
                                                        ...
                                                        if (data.state.reported[prop] == null)
                                                            ...
                                                            thing['reported'][i] = {propName : prop, propValue :data.state.reported[prop]};
                                                        break;
                                                    }
                                                }
                                               if (!foundInReported){
                                                ...
                                                for (var i=0;i<thing.desired.length;i++){
                                                    if (thing.desired[i].propName == prop){
                                                        ...
                                                        if (data.state.desired[prop] == null)
                                                            ...
                                                            thing['desired'][i] = {propName : prop, propValue :data.state.desired[prop]};
                                                        break;
                                                    }
                                                }
                                               if (!foundInDesired){
    ...
    var iot = new awssdk.Iot(options = {region:'us-west-2'})
